...
def generate_csproj_patches(corvax_dst: Path) -> int:
    ...
    that has Aavikko.Content/Mods/<project>/Aavikko/ files.

    These patches add <Compile Include="..\\Aavikko.Content\\Mods\\<project>\\Aavikko\\**\\*.cs" />
    to each csproj so the SDK-style project picks up Aavikko C# files from the overlay.

    Without these patches, the compiler won't see Aavikko.Content/Mods/ files and
    ...
    for proj in projects:
        ...
        if "Aavikko.Content" in content:
            continue  # Already has reference, skip

        # Insert ItemGroup before </Project>
        include_path = f"..\\\\Aavikko.Content\\\\Mods\\\\{proj}\\\\Aavikko\\\\**\\\\*.cs"
...
def migrate_content(aavikko_build: Path, corvax_dst: Path) -> tuple[int, int]:
    """Migrate C# modifications from Aavikko-2.0 to Aavikko.Content/Patches/ + Mods/.
