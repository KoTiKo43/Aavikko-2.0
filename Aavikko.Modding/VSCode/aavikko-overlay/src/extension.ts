...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    if (!buildRoot) {
        log('Aavikko.Modding/Patcher/Apply.py not found — extension inactive');
        return;
    }
    const patcherDir = path.join(buildRoot, 'Aavikko.Modding', 'Patcher');
    ...
    const appliedWatcher = vscode.workspace.createFileSystemWatcher(
        '**/Aavikko.Modding/Patcher/.applied');
    ...
    const overlayWatcher = vscode.workspace.createFileSystemWatcher(
        new vscode.RelativePattern(vscode.Uri.file(buildRoot), '{' + [
            'Aavikko.Resources/Mods/**', 'Aavikko.Resources/Patches/**',
            'Aavikko.Content/Mods/**', 'Aavikko.Content/Patches/**',
            'Aavikko.RobustToolbox/Mods/**', 'Aavikko.RobustToolbox/Patches/**',
            'Aavikko.Modding/Patcher/.upstream_state.json',
            'Aavikko.Modding/Patcher/.conflict_decisions.yml',
        ].join(',') + '}'));
...
function registerCommands(
    ...
    // ── Lifecycle ──
    cmd('aavikko.apply', () => runHeadless('Apply.py', [], 'Applying overlay'));
    cmd('aavikko.clear', () => runHeadless('Clear.py', [], 'Clearing overlay'));
    cmd('aavikko.toggleOverlay', async () => {
        await refresh();
        await runHeadless(state.applied ? 'Clear.py' : 'Apply.py', [],
            state.applied ? 'Clearing overlay' : 'Applying overlay');
    });
    cmd('aavikko.generateAll', () => runHeadless('Generate.py', ['--all'], 'Generating patches'));
    cmd('aavikko.runValidate', () => runHeadless('Validate.py', [], 'Validating overlay'));
    ...
    const generateFor = async (item: unknown, restore: boolean) => {
        ...
        await runHeadless('Generate.py', args, `Capturing ${path.basename(rel)}`);
    };
...
function findOverlayFile(buildRoot: string, rel: string): string | null {
    ...
    if (rel.startsWith('Resources/')) {
        ...
        return first(
            path.join(buildRoot, 'Aavikko.Resources', 'Patches', mirror),
            path.join(buildRoot, 'Aavikko.Resources', 'Mods', mirror));
    }
    if (rel.startsWith('RobustToolbox/')) {
        const inner = rel.substring('RobustToolbox/'.length);
        return first(
            path.join(buildRoot, 'Aavikko.RobustToolbox', 'Patches', `${inner}.patch`),
            path.join(buildRoot, 'Aavikko.RobustToolbox', 'Mods', inner));
    }
    return first(
        path.join(buildRoot, 'Aavikko.Content', 'Patches', `${rel}.patch`),
        path.join(buildRoot, 'Aavikko.Content', 'Mods', rel));
}
...
function convertModToPatch(buildRoot: string, filePath: string): void {
    ...
    if (filePath.startsWith('Resources/')) {
        ...
        src = path.join(buildRoot, 'Aavikko.Resources', 'Mods', rel);
        dst = path.join(buildRoot, 'Aavikko.Resources', 'Patches', rel);
    } else {
        src = path.join(buildRoot, 'Aavikko.Content', 'Mods', filePath);
        dst = path.join(buildRoot, 'Aavikko.Content', 'Patches', filePath);
    }
...
function findBuildRoot(workspacePath: string): string | null {
    for (const candidate of [workspacePath, path.join(workspacePath, '..'), path.join(workspacePath, '..', '..')]) {
        if (fs.existsSync(path.join(candidate, 'Aavikko.Modding', 'Patcher', 'Apply.py'))) {
            return candidate;
        }
    }
...
function searchForMarker(dir: string, maxDepth: number): string | null {
    ...
    if (fs.existsSync(path.join(dir, 'Aavikko.Modding', 'Patcher', 'Apply.py'))) {
        return dir;
    }
