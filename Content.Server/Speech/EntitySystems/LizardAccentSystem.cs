...
public sealed partial class LizardAccentSystem : EntitySystem
{
    ...
    // private static readonly Regex _regexLowerZ = new Regex("з+"); // Aavikko з disable
    // private static readonly Regex _regexUpperZ = new Regex("З+"); // Aavikko з disable
    ...
    private void OnAccent(EntityUid uid, LizardAccentComponent component, AccentGetEvent args)
    {
        ...
        // message = _regexLowerZ.Replace(message, _random.Pick(_replacementsSs));       // для "з+" используются те же замены, что и для "с+" // Aavikko з disable
        // message = _regexUpperZ.Replace(message, _random.Pick(_replacementsSsUpper)); // для "З+" используются те же замены, что и для "С+" // Aavikko з disable
